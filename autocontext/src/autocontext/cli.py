...
# AC-697 slice 7: `show` composes the existing get_run + run_status read
# surfaces. The slice-1 contract pins `show` as the operator-facing
# "render finished run" command; the TS side has shipped it for a
# while, this slice adds the Python equivalent.
@app.command()
def show(
    run_id: str = typer.Argument(...),
...
# AC-697 slice 7: `watch` polls run_status on an interval and breaks
# when the latest generation is terminal (completed or failed). The TS
# implementation reads the same store via the SQLiteStore.getRun
# surface; Python mirrors the polling loop semantics (no WebSocket
# dependency: the CLI is meant to be tail-friendly for scripts).
@app.command()
def watch(
    run_id: str = typer.Argument(...),
    ...
    while True:
        ...
        if latest is not None:
            ...
            if current != last_emit:
                ...
                last_emit = current
            if str(latest["status"]).lower() in {"completed", "failed", "succeeded", "errored"}:
                return
